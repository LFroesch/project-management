...
const EtcPage: React.FC = () => {
  ...
  useEffect(() => {
    if (selectedProject) {
      setStaging(selectedProject.staging || '');
    }
  }, [selectedProject]);
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      setError('Failed to save staging information');
    } finally {
      setSaving(false);
    }
  };
    ...
    <div className="p-8">
      ...
      <div className="bg-white rounded-lg border border-gray-200 p-6">
        ...
        <div className="space-y-6">
          <div>
            <h3 className="font-medium text-gray-800 mb-3">Staging Environment:</h3>
            {isEditing ? (
              <textarea
                value={staging}
                ...
                placeholder="Enter staging environment information..."
              />
            ) : (
              <div className="min-h-48 p-4 bg-gray-50 rounded-md">
                <div className="whitespace-pre-wrap text-gray-700 leading-relaxed">
                  {staging || 'No staging information...'}
                </div>
              </div>
            )}
          </div>
