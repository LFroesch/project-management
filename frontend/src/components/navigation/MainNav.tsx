...
const MainNav: React.FC<MainNavProps> = ({ currentPath, onNavigate, getContrastColor }) => {
  ...
  return (
    <div className="absolute left-1/2 transform -translate-x-1/2">
      <div className="tabs-container p-1">
        <button
          className={`tab-button gap-2 ${isActive('/projects') && !projectDetailsPaths.includes(currentPath) ? 'tab-active' : ''}`}
        ...
        <button
          className={`tab-button gap-2 ${projectDetailsPaths.includes(currentPath) ? 'tab-active' : ''}`}
        ...
        <button
          className={`tab-button gap-2 ${isActive('/discover') ? 'tab-active' : ''}`}
        ...
        <button
          className={`tab-button gap-2 ${isActive('/terminal') ? 'tab-active' : ''}`}
