...
const getIcon = (tabId: string) => {
  ...
  return icons[tabId] || null;
};

const ProjectDetailsNav: React.FC<ProjectDetailsNavProps> = ({ currentPath, onNavigate, getContrastColor }) => {
      ...
      <div className="tabs-container p-1">
        {tabs.map((tab) => (
          <button
            ...
            className={`tab-button whitespace-nowrap gap-2 ${currentTab === tab.id ? 'tab-active' : ''}`}
            style={currentTab === tab.id ? { color: getContrastColor() } : {}}
          >
            {getIcon(tab.id)}
            <span>{tab.label}</span>
          </button>
