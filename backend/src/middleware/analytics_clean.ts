...
export class AnalyticsService {
  ...
  // Legacy method for compatibility with routes - FIXED for proper event data mapping
  static async runQuery(eventType: string, params: any[]) {
    ...
    try {
      switch (eventType) {
        case 'feature_usage':
          return await Analytics.aggregate([
            { $match: { eventType: 'feature_usage', timestamp: { $gte: startDate } } },
            {
              $group: {
                _id: {
                  feature: { $ifNull: ['$eventData.featureName', '$eventData.feature'] }
                },
                usage_count: { $sum: 1 },
        ...
        case 'navigation':
          return await Analytics.aggregate([
            { $match: { eventType: 'navigation', timestamp: { $gte: startDate } } },
            {
              $group: {
                _id: {
                  from: { $ifNull: ['$eventData.navigationSource', '$eventData.from'] },
                  to: { $ifNull: ['$eventData.navigationTarget', '$eventData.to'] }
                },
                count: { $sum: 1 }
              }
            },
        ...
        case 'search':
          return await Analytics.aggregate([
            { $match: { eventType: 'search', timestamp: { $gte: startDate } } },
            {
              $group: {
                _id: { $ifNull: ['$eventData.searchTerm', '$eventData.query'] },
                search_count: { $sum: 1 },
                avg_results: { $avg: { $ifNull: ['$eventData.searchResultsCount', 0] } }
              }
            },
            {
              $project: {
        ...
        case 'error':
          return await Analytics.aggregate([
            { $match: { eventType: 'error', timestamp: { $gte: startDate } } },
            {
              $group: {
                _id: { $ifNull: ['$eventData.errorType', '$eventData.type'] },
                error_count: { $sum: 1 },
        ...
        case 'performance':
          return await Analytics.aggregate([
            { $match: { eventType: 'performance', timestamp: { $gte: startDate } } },
            {
              $group: {
                _id: { $ifNull: ['$eventData.actionType', '$eventData.action'] },
                avg_duration: { $avg: { $ifNull: ['$eventData.duration', 0] } },
                max_duration: { $max: { $ifNull: ['$eventData.duration', 0] } },
                count: { $sum: 1 }
              }
            },
            {
        ...
        case 'ui_interaction':
          return await Analytics.aggregate([
            { $match: { eventType: 'ui_interaction', timestamp: { $gte: startDate } } },
            {
              $group: {
                _id: { $ifNull: ['$eventData.interactionType', '$eventData.type'] },
      ...
      console.error(`Error running query for ${eventType}:`, error);
      return [];
    }
  }
}
